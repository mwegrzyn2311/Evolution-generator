...
public class MapVisualizer {
    ...
    private static String drawFrame(boolean innerSegment) {
        if (innerSegment) {
    ...
    private static String drawHeader(Vector2d lowerLeft, Vector2d upperRight) {
        StringBuilder builder = new StringBuilder();
        builder.append(" y\\x ");
        for (int j = lowerLeft.x; j < upperRight.x + 1; j++) {
    ...
    private String drawObject(Vector2d currentPosition) {
        String result = null;
        if (this.map.containsSomething(currentPosition)) {
